...
import test  
from utils.general import check_dataset, check_file, check_img_size, colorstr, init_seeds, labels_to_class_weights, one_cycle, increment_path, strip_optimizer
...
class Trainer:
    def __init__(self, opt, device) -> None:
        ...
        self.opt.save_dir =  increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok | opt.evolve)  
        ...
        self.accumulate = max(round(self.nominal_batch_size / self.opt.total_batch_size), 1)  # the number of iterations to accumulate gradients before performing a weight update in (SGD) 
        self.hyp['weight_decay'] *= self.opt.total_batch_size * self.accumulate / self.nominal_batch_size  # scale weight_decay
        ...
        self.scheduler, self.lf = self.load_scheduler() #scheduler, learning rate lambda


        # # EMA
        self.ema = ModelEMA(self.model) if self.rank in [-1, 0] else None

        self.dataset, self.dataloader, self.valloader, self.num_layers, self.num_batches, self.imgsz, self.imgsz_test = self.load_dataset()

        # Model parameters
        self.hyp['box'] *= 3. / self.num_layers  # scale to layers
        self.hyp['cls'] *= self.nc / 80. * 3. / self.num_layers  # scale to classes and layers
        self.hyp['obj'] *= (self.imgsz / 640) ** 2 * 3. / self.num_layers  # scale to image size and layers
    ...
    def log_wandb(self, data_dict):
        ...
        if self.rank in [-1, 0]:
            ...
            run_id = torch.load(self.opt.weights, map_location=self.device).get('wandb_id') if self.opt.weights.endswith('.pt') and os.path.isfile(self.opt.weights) else None
    ...
    def load_scheduler(self):
        ...
        if self.opt.linear_lr:
            lf = lambda x: (1 - x / (self.opt.epochs - 1)) * (1.0 - self.hyp['lrf']) + self.hyp['lrf']  # linear
        else:
    ...
    def load_dataset(self):
        ...
        if self.rank in [-1, 0]:
            valloader = create_dataloader(test_path, imgsz_test, opt.batch_size * 2, gs, opt,  # testloader
    ...
    def train(self):
        ...
        model = self.model
        t0 = time.time()
        self.num_warm_up_iters = max(round(self.hyp['warmup_epochs'] * self.num_batches), 1000)  # number of warmup iterations, max(3 epochs, 1k iterations)
        self.maps = np.zeros(self.nc)  # mAP per class
        self.results_list = [(0, 0, 0, 0, 0, 0, 0)]  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
    ...
    def train_on_epoch(self, epoch, model):
        ...
        for i, (imgs, targets, paths, _) in pbar: 
            ...
            self.plot_on_batch(epoch, i, mean_losses, loss_items, pbar, imgs, targets, paths)
    ...
    def warm_up(self, epoch, ni):
        ...
        self.accumulate = max(1, np.interp(ni, xi, [1, self.nominal_batch_size / self.opt.total_batch_size]).round())
    ...
    def train_on_batch(self, ni, model, imgs, targets):
        ...
        with amp.autocast(enabled=self.device != 'cpu'):
            ...
            if 'loss_ota' not in self.hyp or self.hyp['loss_ota'] == 1:
                loss, loss_items = self.compute_loss_ota(pred, targets.to(self.device), imgs)  # loss scaled by batch_size
            else:
    ...
    def plot_on_batch(self, epoch, i,mean_losses, loss_items, pbar, imgs, targets, paths):
        # Print
        if self.rank in [-1, 0]:
            mem = '%.3gG' % (torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
            s = ('%10s' * 2 + '%10.4g' * 6) % (
                '%g/%g' % (epoch, self.opt.epochs - 1), mem, *mean_losses, targets.shape[0], imgs.shape[-1])
            pbar.set_description(s)

            # Plot
            if i < 5 and epoch%5==0:
                f = self.save_dict["save_dir"] / f'train_epoch_{epoch}_batch_{i}.jpg'  # filename
                ...
                self.wandb_logger.log({"Mosaics": [self.wandb_logger.wandb.Image(str(x), caption=x.name) for x in
                                                self.save_dict["save_dir"].glob('train*.jpg') if x.exists()]})
    ...
    def end_epoch(self, epoch, mean_losses):
        ...
        if self.rank in [-1, 0]:
            ...
            # Write
            s = ('%10s' * 2 + '%10.4g' * 3) % ( '%g/%g' % (epoch, self.opt.epochs - 1),  *mean_losses)

            with open(self.save_dict["results_file"] , 'a') as f:
                f.write(s + '%10.4g' * 7 % results + '\n')  # append metrics, val_loss
            if len(self.opt.name) and self.opt.bucket:
                os.system('gsutil cp %s gs://%s/results/results%s.txt' % ( self.save_dict["results_file"] , opt.bucket, opt.name))
            ...
            if (not self.opt.nosave) or (final_epoch and not self.opt.evolve):  # if save
                ...
                if (self.best_fitness == fi) and (epoch >= 200):
                    torch.save(ckpt, self.save_dict["wdir"] / 'best_{:03d}.pt'.format(epoch))
                elif ((epoch+1) % 25) == 0:
                    torch.save(ckpt, self.save_dict["wdir"] / 'epoch_{:03d}.pt'.format(epoch))
                elif epoch >= (self.opt.epochs-5):
                    torch.save(ckpt, self.save_dict["wdir"] / 'epoch_{:03d}.pt'.format(epoch))
                if self.wandb_logger.wandb:
                    if ((epoch + 1) % self.opt.save_period == 0 and not final_epoch) and self.opt.save_period != -1:
                        self.wandb_logger.log_model(
                            self.last.parent, self.opt, epoch, fi, best_model=self.best_fitness == fi)
                del ckpt
    ...
    def end_training(self):
        ...
        if self.wandb_logger.wandb:
            ...
            self.wandb_logger.log({"Results": [self.wandb_logger.wandb.Image(str(self.save_dict["save_dir"] / f), caption=f) for f in files
                                              if (self.save_dict["save_dir"] / f).exists()]})
        
        # Strip optimizers
        final = self.save_dict["best"] if self.save_dict["best"].exists() else self.save_dict["last"]  # final model
        ...
        if self.wandb_logger.wandb and not self.opt.evolve:  # Log the stripped model
            self.wandb_logger.wandb.log_artifact(str(final), type='model',
