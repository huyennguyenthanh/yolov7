...
def train(hyp, opt, device, tb_writer=None):
    ...
    time_dict = {
        ...
        "train_on_batch_foward_one": 0,
        "train_on_batch_foward_two": 0,
        "loss_backward":0,
        "optimizer":0,
        "evaluate":0,
        "scheduler":0,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if not epoch <= hyp["supervised_epoch"] and epoch % 3 == 0:
            ...
            if epoch < hyp["supervised_epoch"]:
                ...
                try: 
                    ...
                    with torch.no_grad():
                        ...
                        pseudo_label_hard = convert_output_to_label_2(unlabel_imgs_weak_aug, pred_hard, conf=True, device=device)
                        pseudo_label_soft = convert_output_to_label_2(unlabel_imgs_weak_aug, pred_soft, conf=True, device=device)
                    ...
                    with amp.autocast(enabled=cuda):
                        ...
                        if hyp['loss_ota'] == 1:
                            s_loss_u, s_loss_items = compute_loss_ota(s_pred_us, pseudo_label_soft.to(device), label_imgs)  # loss scaled by batch_size
                        else:
                            s_loss_u, s_loss_items = compute_loss(s_pred_us, pseudo_label_soft.to(device))  # loss scaled by batch_size
                        ...
                        if opt.quad:
                            semi_loss *= 4.
                            loss *= 4
                        time_dict["train_on_batch_forward_one"] = time.time() - start
                        start = time.time()
                    ...
                    with amp.autocast(enabled=cuda):
                        ...
                        if hyp['loss_ota'] == 1:
                            s_loss_u, s_loss_items = compute_loss_ota(s_pred_us, pseudo_label_hard.to(device), label_imgs)  
                        else:
                            s_loss_u, s_loss_items = compute_loss(s_pred_us, pseudo_label_hard.to(device))  
                        ...
                        time_dict["train_on_batch_foward_two"] = time.time() - start
                        start = time.time()
