...
class ComputeLossOTASemi:
    ...
    def __call__(
        ...
        if obj_only:
            return lobj * bs, torch.cat((lobj, lcls * 0.0, lbox*0.0)).detach()
...
class ComputeLossSemi:
    ...
    def __init__(self, model, autobalance=False):
        super(ComputeLossSemi, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def __call__(self, p, targets, obj_only=False, grad=True):  # predictions, targets, model
        device = targets.device
