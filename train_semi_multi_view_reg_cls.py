...
from utils.semi_psuedo_label_process import convert_output_to_label_2, non_max_suppression_pseudo_decouple_multi_view_reg_cls
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        weights = "/home/nguyen.thanh.huyenb/yolov7/YOLOv7_2_labels/fold_3_percent_10_semi_multui_view2/weights/epoch_004.pt"
        ...
        with torch.no_grad():
            csd_t = ckpt['model_teacher'].float().state_dict()  # checkpoint state_dict as FP32
        ...
        # model_teacher = attempt_load(weights, map_location=device, model_teacher=True) 
    ...
    if opt.adam:
        s_optimizer = optim.Adam(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
        t_optimizer = optim.Adam(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
    else:
        s_optimizer = optim.SGD(pg0, lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
        t_optimizer = optim.SGD(pg0, lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)

    s_optimizer.add_param_group({'params': pg1, 'weight_decay': hyp['weight_decay']})  # add pg1 with weight_decay
    s_optimizer.add_param_group({'params': pg2})  # add pg2 (biases)
    ...
    s_scheduler = lr_scheduler.LambdaLR(s_optimizer, lr_lambda=lf)
    t_scheduler = lr_scheduler.LambdaLR(t_optimizer, lr_lambda=lf)
    ...
    s_compute_loss_ota = ComputeLossOTA(model) 
    s_compute_loss = ComputeLoss(model) 
    s_compute_loss_semi = ComputeLossSemi(model)
    s_compute_loss_ota_semi = ComputeLossOTASemi(model)


    t_compute_loss_ota = ComputeLossOTA(model_teacher)  
    t_compute_loss = ComputeLoss(model_teacher)  
    t_compute_loss_semi = ComputeLossSemi(model_teacher)
    t_compute_loss_ota_semi = ComputeLossOTASemi(model_teacher)
        ...
        for i, data in pbar:  # batch -------------------------------------------------------------
            ...
            if epoch < hyp["supervised_epoch"]:
                ...
                with amp.autocast(enabled=cuda):
                    ...
                    if hyp['loss_ota'] == 1:
                        s_loss, s_loss_items = s_compute_loss_ota(pred, label_targets.to(device), label_imgs)  # loss scaled by batch_size
                    else:
                        s_loss, s_loss_items = s_compute_loss(pred, label_targets.to(device))  # loss scaled by batch_size
                ...
                try: 
                    ...
                    with torch.no_grad():
                        ...
                        pred_reg, pred_cls = non_max_suppression_pseudo_decouple_multi_view_reg_cls(out, conf_thres=cls_threshold, iou_thres=bbox_threshold)
                        pseudo_label_reg = convert_output_to_label_2(unlabel_imgs_weak_aug, pred_reg, conf=True)
                        pseudo_label_cls = convert_output_to_label_2(unlabel_imgs_weak_aug, pred_cls, conf=True)
                     
                        semi_label_items = pseudo_label_reg.shape[0] + pseudo_label_cls.shape[0]

                        if (i % round(nb/5) == 0):
                            f =  save_dir / f"train_epoch_{epoch}_batch_{i}_pseudo_label_reg.jpg"
                            plot_images(unlabel_imgs_weak_aug, unlabel_targets, None, f,conf_thres=0.0, predictions=pseudo_label_reg)
                            f =  save_dir / f"train_epoch_{epoch}_batch_{i}_pseudo_label_cls.jpg"
                            plot_images(unlabel_imgs_weak_aug, unlabel_targets, None, f,conf_thres=0.0, predictions=pseudo_label_cls)
                    

                          
                        unlabel_targets_reg = pseudo_label_reg[:,:6]
                        unlabel_targets_cls = pseudo_label_reg[:,:6]
                    ...
                    with amp.autocast(enabled=cuda):
                        ...
                        if hyp['loss_ota'] == 1:
                            ...
                            s_loss_semi_reg, s_loss_items_semi_reg = s_compute_loss_ota_semi(semi_pred, unlabel_targets_reg.to(device), unlabel_imgs_strong_aug, reg_only=True)  # loss scaled by batch_size
                            s_loss_semi_cls, s_loss_items_semi_cls = s_compute_loss_ota_semi(semi_pred, unlabel_targets_cls.to(device), unlabel_imgs_strong_aug, cls_only=True)

                        else:
                            s_loss, s_loss_items = s_compute_loss(sup_pred, label_targets.to(device))  # loss scaled by batch_size
                            s_loss_semi_reg, s_loss_items_semi_reg = s_compute_loss_semi(semi_pred, unlabel_targets_reg.to(device), reg_only=True)  # loss scaled by batch_size
                            s_loss_semi_cls, s_loss_items_semi_cls = s_compute_loss_semi(semi_pred, unlabel_targets_cls.to(device), cls_only=True) 

                        s_loss_semi = s_loss_semi_reg + s_loss_semi_cls 
                        s_loss_items_semi = s_loss_items_semi_reg + s_loss_items_semi_cls 
            ...
            if rank in [-1, 0] and i % round(nb/5) == 0:
