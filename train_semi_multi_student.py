...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%12s' * 12) % ('Epoch', 'gpu_mem', 't_loss_l', 't_loss_u', 't_loss', 's_loss_l', 's_loss_u', 't_loss_l', 't_loss_u','t_loss', 'labels', 'semi_labels'))
        with open(results_file, 'a') as f:
            f.write( ('%12s' * 12) % ('Epoch', 'gpu_mem', 't_loss_l', 't_loss_u', 't_loss', 's_loss_l', 's_loss_u', 't_loss_l', 't_loss_u', 't_loss', 'labels', 'semi_labels') \
        ...
        for i, data in pbar:  # batch -------------------------------------------------------------
            ...
            if epoch < hyp["supervised_epoch"]:
                ...
                try: 
                    ...
                    with amp.autocast(enabled=cuda):
                        ...
                        if (i % round(nb/20) == 0):
                            f =  save_dir / f"train_epoch_{epoch}_batch_{i}_pseudo_label_t.jpg"
                        ...
                        t_loss = t_loss_l + hyp["semi_loss_weight"] * t_loss_u
                        ...
                        s_loss_l, loss_items = compute_loss(s_pred_l, label_targets.to(device))
                        s_loss_u, s_loss_items = compute_loss(s_pred_us, pseudo_label.to(device))  # loss scaled by batch_size


                        s_loss = s_loss_l + hyp["semi_loss_weight"] * s_loss_u
                    '''3. loss backward
                    '''
                    s_scaler.scale(s_loss).backward()
                    s_scaler.step(s_optimizer)  # optimizer.step
                    ...
                    with amp.autocast(enabled=cuda):
                        ...
                        with torch.no_grad():
                            s_pred_uw = model(unlabel_imgs_weak_aug)

                        out = convert_to_eval_output(model_teacher,s_pred_uw, device=device)
                        pred = non_max_suppression(out, conf_thres=cls_threshold, iou_thres=bbox_threshold)
                        pseudo_label = convert_output_to_label_2(unlabel_imgs_weak_aug, pred, conf=True, device=device).detach()
                        
                        if (i % round(nb/20) == 0):
                            f =  save_dir / f"train_epoch_{epoch}_batch_{i}_pseudo_label_s.jpg"
                            plot_images(unlabel_imgs_weak_aug, unlabel_targets, None, f,conf_thres=0.0, predictions=pseudo_label.detach())
                        ...
                        '''2.2 Student foward on strong
                        '''
                        t_images = torch.cat((label_imgs, unlabel_imgs_strong_aug)) # [64,3,32,32]
                        t_pred = model_teacher(t_images) # [64,10]
                        t_pred_l = [p[:l] for p in t_pred]
                        t_pred_us = [p[l:] for p in t_pred]
                        del t_pred

         
                        t_loss_l, loss_items = compute_loss(t_pred_l, label_targets.to(device))
                        t_loss_u, t_loss_items = compute_loss(t_pred_us, pseudo_label.to(device))  # loss scaled by batch_size
            ...
            if rank in [-1, 0] :
                ...
                s = ('%10s' * 2 + '%12.4g' * 10) % (
                    '%g/%g' % (epoch, epochs - 1), mem, t_loss_l, t_loss_u, t_loss, s_loss_l, s_loss_u, t_loss_l, t_loss_u, t_loss, label_targets.shape[0], semi_label_items)
